...
void dfsGetQu(int t, int x1, int y1, int x, int y) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (ok(map[X][Y]) && !MAP[X][Y] && X >= 0 && Y >= 0 && X <= mapX + 1 &&
        Y <= mapY + 1) {
      MAP[X][Y] = 1;
...
void paint(int x, int y, int t) { putimage(y * 70, x * 55, img[t]); }
...
void goit(int x, int y, Node &t) {
  ...
  if (newx <= mapX + 1 && newx >= 0 && newy <= mapY + 1 && newy >= 0 &&
      ok(map[newx][newy])) {
    paint(t.x + 1, t.y + 1, 1);
    ...
    if (t.t == 5) {
      for (int i = 0; i < mons.size(); i++) {
        ...
        while (qutop[i] - 1 > qutop2[i] && qutop[i] >= 2 && qutop2[i] >= 0)
          if (qutop[i] >= 2 &&
              getok(qu[i][qutop[i]].first, qu[i][qutop[i]].second,
...
void stratgame() {
  ...
  while (1) {
    ...
    for (int i = 0; i < mons.size(); i++) {
      auto elapsed =
          chrono::duration_cast<chrono::milliseconds>(now - lastTime[i]);
      ...
      auto Elapsed =
          chrono::duration_cast<chrono::milliseconds>(now - lasthurt[i]);
      if (Elapsed.count() >= mons[i]->q && player.x == mons[i]->x &&
          player.y == mons[i]->y) {
...
int main() {
  ...
  string lujing[100]{
      "beijing.jpg",   "caofangkuai.jpg",
      "nitu.jpg",      "chuansongmenfangkuai.jpg",
      "wanjia.jpg",    "shenglitupian.jpg",
      "yuanshi.jpg",   "zuanshikuang.jpg",
      "tiekuang.jpg",  "jiangshi.jpg",
      "jiyan.jpg",     "kongbai.jpg",
      "shibai.jpg",    "xin.jpg",
      "kongxin.jpg",   "gong.jpg",
      "jian.jpg",      "jiqishi.jpg",
      "nishi.jpg",     "yujie.jpg",
      "zhudating.jpg", "maoxianmoshi.jpg",
  };
