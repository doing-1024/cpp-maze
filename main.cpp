...
void initGame() {
	...
	lasthp = chrono::system_clock::now();
}
...
void do_game() {
	...
	for(char x = '1';x <= '9';x++)
		if (GetAsyncKeyState(x) & 0x0001) At = x - '1';
	if (GetAsyncKeyState('0') & 0x0001) At = 9;
	...
	if (player.x == mapX + 1 && player.y == mapY + 1) { maxhp += guaiShu; resultImgTag = obc::win; STATE = 2; }
	else if (player.hp <= 0) { resultImgTag = obc::loss; STATE = 2; maxhp -= guaiShu; }
}
...
int main() {
	...
	wupin[111] = {0, 0, obc::mujianblack, 0, 1, 0, 0, 0};
	wupinlan[0] = 111;
	...
	for (int i = 1; i <= 25; i++) delimage(img[i]);
	closegraph();
	cout << "资源释放完毕";
	return 0;
}
